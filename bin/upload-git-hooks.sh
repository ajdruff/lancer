...
scp ${DIR%%/}/config-bash.conf ${SSH_CONNECTION}:.git/hooks/config-bash-advanced.conf;
...
echo 'chmod 700 .git/hooks/config-bash-advanced.conf' | ssh ${SSH_CONNECTION} 'bash -s';
